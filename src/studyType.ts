...
interface Rule {
  type: StudyType;
  pattern: RegExp;
  /** 需要醒目標示的高階證據（指引與共識），列表上用實心 badge。 */
  strong?: boolean;
}

const RULES: Rule[] = [
  { type: "指引", pattern: /\b(clinical practice guideline|practice guideline|guidelines?)\b/i, strong: true },
  { type: "共識", pattern: /\b(consensus|position (statement|stand)|delphi)\b/i, strong: true },
  ...
  { type: "敘述性回顧", pattern: /\b(narrative review|review)\b/i },
];
...
export function isStrongEvidence(type: StudyType | null): boolean {
  return type === "指引" || type === "共識";
}
