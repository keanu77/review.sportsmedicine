...
import { fixture, fixtureDraft, primaryReview } from '../server/helpers.mjs';
...
test('owner approved revision flows through worker rendering to authenticated ZIP download',async()=>{
  ...
  try {
    ...
    const completed=await f.call(`/jobs/${job.id}/complete`,{method:'POST',role:'worker',data:{leaseToken:research.leaseToken,draft,metadata:{paper:{title:'Fixture study',citation:'Fixture study. doi:10.1234/example',doi:'10.1234/example',sourceUrl:'https://doi.org/10.1234/example',license:null,fullTextVerified:true,sha256:'fixture'},reviews:[primaryReview([{severity:'medium',claim:'標題過度推論',reason:'原文只說相關',suggestion:'改為相關',locator:'',quote:''}])]},artifacts:['source']}});
    assert.equal(completed.status,200);
    // Gate A (claims) and gate B (every primary finding settled) before rendering.
    await f.approve(job.id);
    const {runs}=await (await f.call(`/jobs/${job.id}/reviews`)).json();
    assert.equal((await f.call(`/jobs/${job.id}/reviews/${runs[0].id}/findings/codex/0`,{method:'PATCH',data:{status:'rejected',reason:'標題已寫「相關」'}})).status,200);
    const ready=(await (await f.call(`/jobs/${job.id}`)).json()).job;
    ...
    const config={origin:'https://review.example.com',token:f.env.WORKER_TOKEN,workspace,provider:'claude'};
    ...
    assert.equal(strFromU8(files['fb-post.md']),withDisclaimer(draft.post));
    assert.match(strFromU8(files['review-rejections.md']),/標題過度推論[\s\S]*駁回理由：標題已寫「相關」/);assert.equal(files['paper.pdf'],undefined);
