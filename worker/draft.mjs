...
export function validateEvidence(draft, text) {
  ...
  for (const claim of draft.claims) {
    const match = /^p:(\d+)$/.exec(claim.locator);
    const quote = normalizeQuote(String(claim.quote ?? ''));
    if (!claim.text?.trim() || quote.length < 12 || !match || !pages[Number(match[1]) - 1] || !normalizeQuote(pages[Number(match[1]) - 1]).includes(quote)) throw new Error(`研究依據無法定位原文：${claim.locator ?? '缺少頁碼'}，請核對草稿`);
  }
  return draft;
}
...
export async function generateDraft(source, directory, { signal, provider = process.env.REVIEW_MODEL_PROVIDER ?? 'codex', allowRetry = false } = {}) {
  ...
  if (source.text.length > 160000) throw new Error('全文超過第一版單篇處理上限；需要分段全文分析，未產生摘要替代稿');
  const inputHash = createHash('sha256').update(source.text).update(provider).digest('hex');
  const resultFile = path.join(directory, 'draft.json'), requestFile = path.join(directory, 'draft-request.json');
  try { const saved = JSON.parse(await readFile(resultFile, 'utf8')); if (saved.sourceHash === inputHash) return validateEvidence(saved.draft, source.text); } catch {}
  if (!allowRetry) {
  ...
  validateEvidence(draft, source.text);
