...
- [x] Integration: same-origin dev API bridge, local Cloudflare setup, production configuration guide, Mac doctor and foreground/startup commands, `.gitignore` for secrets/workspaces. Run build, unit/integration tests, current smoke tests and real single-paper workflow.
...
Download follow-up: the user reported Edge's native attachment download failed. Commit `4eaca6f` is deployed and adds same-origin fetching, byte-length/SHA-256 verification, visible errors, and a retained explicit save link. All 15 frontend tests and 43 browser tests pass, including saving and comparing actual fixture ZIP bytes. The user confirmed the production version-7 ZIP downloaded successfully after clicking the new save link. The completed `Downloads/social-materials.zip` is 3,034,885 bytes with the expected SHA-256 and all 15 ZIP entries passing CRC verification. Integration acceptance is complete. See the verification document for deployment and artifact evidence.
