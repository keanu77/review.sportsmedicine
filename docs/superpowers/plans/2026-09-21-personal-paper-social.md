...
Download follow-up: the user reported Edge's native attachment download failed. Commit `4eaca6f` is deployed and adds same-origin fetching, byte-length/SHA-256 verification, visible errors, and a retained explicit save link. All 15 frontend tests and 43 browser tests pass, including saving and comparing actual fixture ZIP bytes. The production version-7 ZIP arrived intact in Edge's download temporary file (3,034,885 bytes; hash and CRC verified); native final save remains pending user confirmation. Keep Integration unchecked until this last step is confirmed. See the verification document for deployment and artifact evidence.
